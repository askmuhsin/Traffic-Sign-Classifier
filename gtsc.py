...
k_value = np.random.randint(len(X_train))
print(k_value)
plt.imshow(X_train[k_value].squeeze())
print(y_train[k_value])
...
def imagePreprocess(image_set):
    ...
    print("Image data set dimension :", np.shape(img_train))
    print("Dimension after converting to grayscale :", np.shape(X_gray))

    k_value = np.random.randint(len(img_train))

    plt.subplot(121)
    plt.imshow(img_train[k_value])
    plt.title('Original Image\n(randomly chosen)')

    plt.subplot(122)
    plt.imshow(X_gray[k_value].squeeze(), cmap='gray')
    ...
    print("Arithmetic mean of image set :", np.mean(X_gray))
    print("Arithematic mean after normalization:", np.mean(X_norm))
    ...
    plt.subplot(122)
    plt.imshow(X_norm[k_value].squeeze(), cmap='gray')
    plt.title('Normalized equivalent')
    plt.show()
...
print ("#####################################################################")
################################################################################
print ("Running test set for accuracy...")
...
plt.show()
labelWeb = [14,28,8,18,13] #change this label values if the images in directory are changed
